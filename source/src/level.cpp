...
void Level::loadMap(std::string mapName, Graphics &graphics) {
	...
	//Get the width and the height of the map and store it in _size
	int width, height;
				...
				while (pData) {
					...
					if (pTile != NULL) {
						...
						while (pTile) {
							...
							this->_tileLists.push_back(tile);
							tileCounter++;
	...
	if (pObjectGroup != NULL) {
		while (pObjectGroup) {
			...
			//Other objectgroups go here with an else if ((ss.str() == "name")
...
void Level::draw(Graphics &graphics) {
	for (int i = 0; i < this->_tileLists.size(); i++) {
		this->_tileLists.at(i).draw(graphics);
	}
}
